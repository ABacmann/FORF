...
def main():
    ...
    # Select the appropriate generator model
    ...
    # Load image and mask
    image = Image.open(args.image).convert('RGB')  # Ensure image is in RGB mode
    mask = Image.open(args.mask).convert('L')  # Ensure mask is in grayscale mode
    ...
    img_out = ((image_inpainted[0].permute(1, 2, 0) + 1) * 127.5).clamp(0, 255).to(device='cpu', dtype=torch.uint8)
