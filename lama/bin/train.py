...
def main(config: OmegaConf):
    try:
        ...
        # Set gpus to 0 if CUDA is not available
        if not torch.cuda.is_available():
            trainer_kwargs['gpus'] = 0
            trainer_kwargs['accelerator'] = 'ddp_cpu'

        # Check if training dataset is empty
        train_loader = make_default_train_dataloader(**config.data.train)
        if len(train_loader.dataset) == 0:
            raise ValueError("Training dataset is empty. Please ensure the 'train' directory contains images.")
        print(f"Number of samples in the training dataset: {len(train_loader.dataset)}")

        # Update val_check_interval
        trainer_kwargs['val_check_interval'] = min(trainer_kwargs['val_check_interval'], 1000)
